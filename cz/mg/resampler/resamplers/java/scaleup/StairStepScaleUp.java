...
public class StairStepScaleUp implements NonuniformScaleUp {
    private float stepScale = 1.5f;
    private Stretch stretch = new BoxStretch();

    public StairStepScaleUp() {
    }

    public StairStepScaleUp(float stepScale, Stretch stretch) {
    ...
    public void scaleUp(Image sourceImage, Image destinationImage, Progress progress) throws Exception {
        if(destinationImage.getWidth() != getResultWidth(sourceImage.getWidth())) throw new Exception("Invalid destination width for stair step scale up. Expected " + getResultWidth(sourceImage.getWidth()) + ", got " + destinationImage.getWidth() + ".");
        if(destinationImage.getHeight() != getResultHeight(sourceImage.getHeight())) throw new Exception("Invalid destination height for stair step scale up. Expected " + getResultHeight(sourceImage.getHeight()) + ", got " + destinationImage.getHeight() + ".");
    ...
    public void scaleUpHorizontally(Image sourceImage, Image destinationImage, Progress progress) throws Exception {
        if(destinationImage.getWidth() != getResultWidth(sourceImage.getWidth())) throw new Exception("Invalid destination width for stair step scale up. Expected " + getResultWidth(sourceImage.getWidth()) + ", got " + destinationImage.getWidth() + ".");
        if(destinationImage.getHeight() != sourceImage.getHeight()) throw new Exception("Invalid destination height for stair step scale up. Expected " + sourceImage.getHeight() + ", got " + destinationImage.getHeight() + ".");
        scale(sourceImage, destinationImage, progress);
    }

    @Override
    public void scaleUpVertically(Image sourceImage, Image destinationImage, Progress progress) throws Exception {
        if(destinationImage.getWidth() != sourceImage.getWidth()) throw new Exception("Invalid destination width for stair step scale up. Expected " + sourceImage.getWidth() + ", got " + destinationImage.getWidth() + ".");
        if(destinationImage.getHeight() != getResultHeight(sourceImage.getHeight())) throw new Exception("Invalid destination height for stair step scale up. Expected " + getResultHeight(sourceImage.getHeight()) + ", got " + destinationImage.getHeight() + ".");
        scale(sourceImage, destinationImage, progress);
    }
